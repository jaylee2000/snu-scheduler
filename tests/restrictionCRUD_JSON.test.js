...
beforeEach( async () => {
	await request(app).post("/restriction").send(restrictionA_input).expect(302); // redirect
})
...
test('Read all restrictions', async () => {
	...
	await request(app).post("/restriction").send(restrictionB_input).expect(302); // redirect
...
test('Update a restriction', async () => {
	...
	await request(app).patch(`/restriction/${A._id}`).send(updateDetails).expect(302); // redirect
	
	const updatedRestrictionFail = await Restriction.find(restrictionA_stored);
	expect(updatedRestrictionFail.length).toBe(0);
	
	const updatedRestrictionSuccess = await Restriction.find(updateDetailsStored);
